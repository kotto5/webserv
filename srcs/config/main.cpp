...
int main()
{
	std::string filepath = "../../conf/error_file/server_in_server.conf";
	...
	std::map<std::string, std::vector<ServerContext> >::const_iterator it = http_block.getServers().begin();
	std::map<std::string, std::vector<ServerContext> >::const_iterator ite = http_block.getServers().end();
	while (it != ite)
	{
